...
public class SparqlQueryExecuter {
	...
	public List<DBpediaResource> query(String query) throws IOException, OutputException, SparqlExecutionException {
		...
		//SpotlightLog.debug(SparqlQueryExecuter.class, "--SPARQL QUERY: %s" , query.replace("\n", " ")); 
...
//            uris = parse(readOutput(get(url)));
	...
	//SpotlightLog.debug(SparqlQueryExecuter.class, "-- %s found.", uris.size());
        return uris;
	}
    ...
    public String request(URL url) throws SparqlExecutionException {
		...
		//SpotlightLog.debug(SparqlQueryExecuter.class,"SparqlQuery failed: %s", method.getStatusLine());
